...
const AICommanderPanel: React.FC<AICommanderPanelProps> = ({ creditor, recommendation, onDeploy }) => {
  ...
  return (
    <motion.div className="ai-commander-panel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <h3>Creditor: {creditor}</h3>
      <p>Recommended Action: {recommendation.action}</p>
      <p>Reason: {recommendation.reason}</p>
      <div className="threat-score-bar" style={{ width: `${threatScore}%`, backgroundColor: threatScore >= 80 ? 'red' : threatScore >= 50 ? 'yellow' : 'green' }}>
